// P:\git\buaty\7\src\app\cart\ClientCart.tsx
"use client";
...
export default function ClientCart({ initialCart }: { initialCart: CartItem[] }) {
  ...
  // Client par load hone ke baad isMounted true ho jayega
  useEffect(() => {
    setIsMounted(true);
  }, []);

  // Hydration fix: Jab tak client mount na ho, server wala 'initialCart' dikhao
  ...
  const displayTotalPrice = isMounted 
    ? totalPrice 
  ...
  return (
    <div className="bg-[#fcfcfc] min-h-screen py-12 md:py-20 px-4 sm:px-6">
      <div className="max-w-[1200px] mx-auto">
        <h1 className="text-3xl md:text-4xl font-black uppercase tracking-widest text-gray-900 mb-10 border-b border-gray-200 pb-6">
          Shopping Bag ({displayTotalItems})
        </h1>
            ...
            {displayItems.map((item) => (
              <div key={item.id} className="flex gap-6 bg-white p-4 rounded-xl shadow-sm border border-gray-100 items-center">
                ...
                <div className="flex-1">
                  <h3 className="font-bold text-gray-900 line-clamp-1 hover:text-[#d81b60] transition-colors">
            ...
            <div className="bg-white rounded-2xl shadow-lg border border-gray-100 p-8 sticky top-24">
              ...
              <div className="space-y-4 text-sm font-medium text-gray-600 mb-6">
                <div className="flex justify-between">
                  <span>Subtotal</span>
                  <span className="font-bold text-gray-900">₹{displayTotalPrice.toLocaleString()}</span>
                </div>
                <div className="flex justify-between">
                  <span>Shipping</span>
                  <span className="text-green-600 font-bold uppercase text-xs tracking-wider bg-green-50 px-2 py-1 rounded">Free</span>
                </div>
                <div className="flex justify-between">
              ...
              <div className="border-t border-gray-100 pt-6 mb-8 flex justify-between items-end">
                ...
                <span className="text-3xl font-black text-[#d81b60]">₹{displayTotalPrice.toLocaleString()}</span>
              </div>
